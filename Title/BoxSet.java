...
public class BoxSet extends Title{
		 ...
		 do {
			...
			try {// insert in the database
				Statement statement = conn.createStatement();
						//query
				statement.executeUpdate("INSERT INTO title(titleTypes,	title, year,genre,director,band,formats,venue,ageRating,actors)VALUES ('BOXSET','" + titleName2 + "', '" + year2 + "','" + genre2  + "','"+director2+"','NOT REQUIRED','"+format2+"','NOT REQUIRED','"+ageRating2+"','NOT REQUIRED') ");
	...
	public void searchTitle() { // this method it will search for the title and display the table from the database
         	...
         	try {
			String query = "SELECT * FROM `title` WHERE titleTypes='BOXSET' ORDER BY `title` ASC  ";
			Statement statement = conn.createStatement();
